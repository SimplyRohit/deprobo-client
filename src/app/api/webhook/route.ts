...
import { QuickNodeTransaction } from "@/lib/types";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!Array.isArray(parsed)) {
      console.warn("Invalid body: not an array");
      return NextResponse.json({ status: 200, message: "Invalid payload" });
    }
    transactions = parsed;
  } catch (err) {
    console.error("Failed to parse JSON:", err);
    return NextResponse.json({ status: 200, message: "Malformed JSON" });
  }
  ...
  for (const tx of transactions) {
    try {
      ...
      switch (decoded.name) {
        ...
        default:
          console.log(`[${tx.signature}] Unknown event: ${decoded.name}`);
      }
    } catch (err) {
      console.error(`[${tx.signature}] Processing error:`, err);
    }
  }
